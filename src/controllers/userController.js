...
exports.createUser = async (req, res) => {
  try {
    ...
    user.token = jwt.sign({ id: newUser._id.toString(), email: user.email, role: user.role, school: user.school }, process.env.JWT_SECRET, { expiresIn: '30d' });
...
exports.login = async (req, res) => {
  try {
    ...
    // Generar el token JWT
    user.token = jwt.sign({ id: findUser._id.toString(), email: user.email, role: user.role, school: user.school }, process.env.JWT_SECRET, { expiresIn: '30d' });
...
async function updateUser(req, res) {
  try {
    const { username, password, new_password, email, faculty, group, year, role, school, activated } = req.body;
    ...
    if (school && school != findUser.school) data.school = school;
    if (req.user.targetId) {
      if(role && role.trim() !== '' && role != findUser.role) data.role = role;
      if(activated && activated != findUser.activated) data.activated = activated;
    }
    ...
    if(req.user.targetId) {
      ...
      user.token = jwt.sign({ id: req.user.id, email: user.email, role: user.role, school: user.school }, process.env.JWT_SECRET, { expiresIn: '30d' });
   }
